...
10. **Campaign Context Layer**: A `festivals/` workspace commonly lives inside a campaign, a git-native workspace AND memory layer (managed by the `camp` CLI) that also holds projects, intents, and cross-festival context. Festival is the execution layer that runs within the workspace; when a campaign is present it supplies the surrounding workspace and durable memory. fest also works standalone in a bare `festivals/` workspace (`fest init`), with no campaign required
